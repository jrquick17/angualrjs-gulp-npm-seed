...
(function () {
    ...
    UserService.$inject = ['DataManagementService', 'DefaultVariableService', 'EnvironmentService', 'LoginService', 'RequestManagementService'];

    function UserService(DataManagementService, DefaultVariableService, EnvironmentService, LoginService, RequestManagementService) {
        ...
        function getUser(user) {
            if (!DefaultVariableService.isObject(user)) {
                ...
                if (userId) {
                    ...
                    return LoginService.getActiveUser().then(function (activeUser) {
                        if (activeUser) {
        ...
        function loadUser(user) {
            ...
            options = RequestManagementService.setParams(options, ['Images', 'Locations.States', 'Tags', 'UserTypes']);

            return DataManagementService.request(options).then(function (data) {
                if (data) {
                    var users = DefaultVariableService.getArray(data.users);
...
(function () {
    ...
    angular.module('myApp').controller('PricingController', PricingController);

    PricingController.$inject = ['DefaultVariableService', 'EnvironmentService', 'UserDurationService', 'UserService'];

    function PricingController(DefaultVariableService, EnvironmentService, UserDurationService, UserService) {
        ...
        function loadUser() {
            return UserService.getUser().then(function (user) {
                if (user) {
                    ...
                    PricingController.totalWeeks = EnvironmentService.get('totalWeeks', 1);
        ...
        function reset() {
            ...
            PricingController.title = DefaultVariableService.getString(PricingController.title, 'PROGRAM PROGRESSION');
...
(function () {
    ...
    angular.module('myApp').directive('pricing', pricing);
